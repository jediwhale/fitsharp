...
namespace fitSharp.Machine.Engine {
    public class ParseType<T,P>: Operator<T, P>, ParseOperator<T> where P: class, Processor<T> {
        ...
        public TypedValue Parse(Type type, TypedValue instance, Tree<T> parameters) {
            ...
            var namedType = Processor.ApplicationUnderTest.FindType(new GracefulNameMatcher(typeName));
            return new TypedValue(type == typeof (RuntimeType) ? new RuntimeType(namedType) : (object)namedType, type);
        }
    }
}
